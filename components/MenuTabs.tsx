...
export default function MenuTabs() {
  ...
  return (
    <div>
      {/* Tab bar */}
      <div className="sticky top-16 z-10 bg-cream border-b border-charcoal/10 overflow-x-auto scrollbar-none">
        <div className="flex min-w-max px-4 sm:px-6">
          {SECTIONS.map((s) => (
            <button
              key={s.id}
              onClick={() => setActive(s.id)}
              className={`font-heading text-xs px-4 py-4 border-b-2 transition-colors whitespace-nowrap ${
                active === s.id
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        {/* Section header */}
        <div className="mb-8">
          <h2 className="font-display text-5xl text-olive">{section.label}</h2>
          {section.subtitle && (
            <p className="font-body text-sm text-charcoal/60 mt-2">{section.subtitle}</p>
          )}
        </div>
        ...
        {/* Section-level add-ons */}
        {section.addOns && (
          <div className="mt-8 pt-6 border-t border-charcoal/10">
            <span className="font-heading text-xs text-charcoal/40 mr-2">Add Ons</span>
            <span className="font-body text-sm text-charcoal">{section.addOns}</span>
          </div>
        )}
      </div>

      {/* Dietary key */}
      <div className="border-t border-charcoal/10 bg-charcoal/5 px-6 py-6">
        <div className="max-w-3xl mx-auto flex flex-wrap items-center gap-4">
          ...
          <span className="font-body text-xs text-charcoal/50 ml-auto">
